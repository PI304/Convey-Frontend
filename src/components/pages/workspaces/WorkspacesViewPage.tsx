...
export const WorkspacesViewPage = () => {
  ...
  const requestDeletePackageInWorkspace = async (packageId: number) => {
    if (id === undefined) return;
    await _deletePackageInWorkspace.mutateAsync([+id, packageId]);
  };
  ...
  const requestRoutineDetails = async () => {
  ...
  return (
    <div css={Container}>
          ...
          {workspace?.surveyPackages.length ? (
            <div css={Tags}>
              {workspace.surveyPackages.map((_package, i) => (
                <div css={Tag} onClick={() => requestDeletePackageInWorkspace(_package.id)} key={i}>
                  <div>{_package.id}</div>
      ...
      <Modal
        ...
        onSubmit={requestRoutineDetails}
